...
class FramesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train and os.path.isdir(path):
            ...
            video_array2 = read_video(path, frame_shape=self.frame_shape)
            num_frames2 = len(video_array2)
            frame_idx2 = np.sort(np.random.choice(num_frames2, replace=True, size=2)) if self.is_train else range(
                num_frames2)
            video_array2 = video_array2[frame_idx]
