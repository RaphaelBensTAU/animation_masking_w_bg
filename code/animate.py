...
def animate(config, generator, mask_generator, checkpoint, log_dir, dataset):
    ...
    for it, x in tqdm(enumerate(dataloader)):
        with torch.no_grad():
            ...
            background_img = torch.from_numpy(img_as_float32(io.imread('beach_background.jpg')).transpose(2, 0, 1)).cuda()
            ...
            for frame_idx in range(driving_video.shape[2]):
                ...
                # driving_refined_mask = (out['fixed_mask']*255).reshape((256,256))
                # driving_refined_mask[driving_refined_mask > 25] = 255
                # driving_refined_mask[driving_refined_mask <= 25] = 0
                # # driving_refined_mask /= 255

                # a = (1-out['second_phase_prediction']) * (driving_refined_mask) + (255-driving_refined_mask)*background_img
                # arr = a.data.cpu().numpy().astype(np.uint8).transpose(0, 2, 3, 1)
                ...
                visualization = Visualizer(**config['visualizer_params']).visualize(source=source_frame, driving=driving_frame, driving2=None, target=None, out=out, background_changed=True, change=arr)
